...
function renderData(){
    ...
    for(var i=0;  i<list.length; i++){
        parent.innerHTML += `<ul><li>${list[i].task}</br><span class='chip'>${list[i].date}</span><button onclick="delTask()" id="del">${"Delete"}</button><button onclick="editask()" id="edit">${"Edit"}</button></li></ul>`;
    }
}
"user"
var un = [];
...
window.editask = function(){
  // const nces = ref(database, `todotask/`);
  update(ref(database, "todotask/" + inp.value)),{
    task: inp.value,
  }
  .then(()=>{
    alert("data removed successfully ")
